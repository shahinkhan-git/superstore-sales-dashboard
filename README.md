...
[![Dashboard Demo](screenshots/dashboard-demo.gif)](https://app.powerbi.com/view?r=eyJrIjoiOTkzZTgxZDQtOTUxNC00NWVhLTliOTUtMjBjZDk4NjlmODM1IiwidCI6ImJhY2JkYWMyLWFkODAtNGM3NC04NTk2LWQyZDA4NjExNWYwMyJ9)
...
Top 10 products by sales, top 10 products by profit, sub-category performance matrix with conditional formatting, sales vs profit scatter by sub-category. Drill-through from Page 2.
...
- Changed `Order Date` and `Ship Date` from Text → Date type
- Changed `Postal Code` from Number → Text (prevents leading zeros being stripped)
...
Measures were organised into three dedicated tables:
- `Core Sales Measures`
...
2. **Expand Technology product range** — highest margin category with consistent growth, best ROI potential
3. **Replicate West region strategy in Central** — weakest region, significant untapped revenue potential
4. **Launch Q1 and Q2 promotions** — address seasonal revenue dip in first half of year
5. **Review or discontinue Tables** — negative profit sub-category, consider repricing or removal
6. **Invest in California and New York retention** — top two states, loyalty programmes would protect this revenue base
...
Building the hamburger navigation using Bookmarks taught me how Power BI manages visual state. Every element's visibility must be intentionally set before capturing a bookmark — learned through several failed iterations.

**Design matters as much as data**
A technically correct dashboard that looks unprofessional loses credibility. Investing time in consistent colour usage, typography and layout alignment significantly elevated the final output.

**Drill-through adds real analytical depth**
Right-clicking a region to drill directly into product-level detail transforms a static report into a genuine investigative tool.
...
**Shahin Khan**
Data Analyst & Business Development Manager — Madrid, Spain
